...
export const App: React.FC = () => {
    ...
    const handleSetImageList = useCallback((imageList: ImageObject[] | string) => {
        setImageList((prev) => {
            // return origin data when change category in sidebar
            if (typeof imageList === 'string' && imageList === 'origin data') {
                return [...data]
    ...
    useLayoutEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get('https://srv.3dcommerce.studio/public/shop/getProducts?shop=6412c8e785fad7b71567f17b');
    ...
    return (
        <div className={styles.app}>
            <Context.Provider value={{ 
                ...
                <ImageDrawer data={data} selectedCategory={selectedCategory} imageList={imageList}></ImageDrawer>
